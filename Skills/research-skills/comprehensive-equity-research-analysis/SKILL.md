...
- 預設將最終報告寫入 `Reports/<YYYY-MM-DD>/comprehensive-research-<ticker>-<YYYY-MM-DD>-00.md`，並在需要時建立該日期資料夾；除非使用者另行指定路徑或檔名。
- 同一 `ticker` 與 `YYYY-MM-DD` 若該日期資料夾內已存在先前版本，依序改用 `-01`、`-02`、`-03`...；寫檔前先檢查 `Reports/<YYYY-MM-DD>/` 內的現有檔名並選擇下一個可用序號，序號固定為兩位數。
...
tags:
  - Reports
  - Reports/<YYYY-MM-DD>
  - Skills/research-skills/comprehensive-equity-research-analysis
  - research-flow/study
