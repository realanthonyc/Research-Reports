...
- 額外建立 `事件雷達`：檢查未來 14-30 天內是否有高重要性事件（例如財報、產品發表會、技術大會、Investor Day、監管裁決、Fed/政策節點、大客戶資本開支更新）。
- 若未來 14 天內有高重要性事件，必須在報告中說明：`市場目前在期待什麼`、`爭議焦點是什麼`、`哪些內容屬於已反映`、`什麼結果才算超預期/低於預期`。
- 若過去 14 天內剛發生高重要性事件，必須做 `事件後深讀`：不只列出事件名稱，而要拆解 `實際發布了什麼`、`沒有發布什麼`、`市場如何解讀`、`你是否同意這種解讀`、`對 Bull/Base/Bear 哪個情境影響最大`。
...
- 若 14 天內有即將發生或剛發生的重要事件，報告必須加入專節，明確回答：
- 1) `市場原本在期待什麼 / 爭論什麼`
- 2) `事件實際說了什麼 / 沒說什麼`
- 3) `市場如何解讀`
- 4) `你是否同意市場解讀，以及理由`
...
- 完成報告與相關輸出後，自動以非互動方式建立 git commit；除非使用者明確要求不要 commit。
- commit 前僅納入本次任務直接產生或修改的相關檔案，避免混入無關變更。
...
2. 客戶與供應商集中度
3. 最新動向與消息評級（含確定性、事件雷達、分析師評價與市場預期）
4. 財務三表重點與比率趨勢（歷史 + 前瞻）
5. 估值（相對估值 + DCF 三情境）
6. 風險與催化（機率/影響/監測指標）
7. 同業比較與關聯標的
...
- 不可只列出事件名稱；對重大事件必須交代 `市場預期`、`實際內容`、`落差`、`市場解讀`、`你的判斷`。
- 若公司所屬產業高度受事件驅動（例如 GTC、WWDC、AI Day、FDA、資本市場日），漏掉近期待發生或剛發生的重要事件視為報告缺陷。
- 結尾加入「非投資建議」聲明。
